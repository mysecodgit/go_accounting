...
func (r *invoicePaymentRepo) Create(payment InvoicePayment) (InvoicePayment, error) {
	result, err := r.db.Exec("INSERT INTO invoice_payments (transaction_id, date, invoice_id, user_id, account_id, amount, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
		payment.TransactionID, payment.Date, payment.InvoiceID, payment.UserID, payment.AccountID, payment.Amount, payment.Status)
...
func (r *invoicePaymentRepo) GetByInvoiceID(invoiceID int) ([]InvoicePayment, error) {
	rows, err := r.db.Query("SELECT id, transaction_id, date, invoice_id, user_id, account_id, amount, status, createdAt, updatedAt FROM invoice_payments WHERE invoice_id = ? ORDER BY createdAt DESC", invoiceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
