...
type PersonRepository interface {
	...
	UpdateNameAndPhone(name string, phone string, id int) (Person, error)
...
func (r *personRepo) UpdateNameAndPhone(name string, phone string, id int) (Person, error) {
	var person Person
	_, err := r.db.Exec("UPDATE people SET name=?, phone=?, updated_at=NOW() WHERE id=?",
		name, phone, id)
...
func (r *personRepo) TypeIDExists(typeID int) (bool, int, error) {
	var buildingID int
	err := r.db.QueryRow("SELECT building_id FROM people_types WHERE id = ?", typeID).
		Scan(&buildingID)
